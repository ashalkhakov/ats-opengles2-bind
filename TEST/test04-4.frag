...
void main( void )
{
  ...
  float inside = step(0.0, F); // No AA here

  // Compute iso-contours (without AA) to show the full distance field
  float isolines = mod(F, 1.0);
  float gridstep = length(fwidth(uv)); // Plain isotropic AA here
  isolines = mix(isolines, inside, gridstep*2.0); // Fade out when distant

  // Final fragment color
  gl_FragColor = vec4(inside, isolines, inside, 1.0);
}
