...
void main( void )
{
  ...
  // 'grid' is a texel grid overlay
  float gridstep = length(fwidth(uv)); // Plain isotropic AA here
  vec2 griduv = smoothstep(0.0, gridstep, min(uvlocal, 1.0-uvlocal));
  float grid = griduv.x * griduv.y;
  grid = mix(grid, 1.0, gridstep*2.0); // Fade out when distant

  // Final fragment color
  gl_FragColor = vec4(grid, pattern, grid, 1.0);
}
