(*
** A simple OpenGL ES 2.0 example: draw a triangle with X/EGL and OpenGL ES 2.x.
** Translated from one of the Mesa examples.
**
...
val attr_color = (GLuint)(uint_of 1)
...
in
  ...
  make_z_rot_matrix (view_rotx, !p_rot) where {
...
extern
fun create_shaders (): void = "create_shaders"
implement create_shaders () = let
   val fragShaderText =
   ...
   val () = () where {
     val (fpf | str) = string_takeout_ptr (fragShaderText)
     val () = shader_from_source (fragShader, str)
     prval () = fpf (str)
   } // end of [where]
   var vertShader = glCreateShader GL_VERTEX_SHADER
   val () = () where {
     val (fpf | str) = string_takeout_ptr (vertShaderText)
     val () = shader_from_source (vertShader, str)
     prval () = fpf (str)
   } // end of [where]
  ...
  val () = glBindAttribLocation (program, attr_color, "color")
  val () = glLinkProgram program // needed to put attribs into effect
  val () = () where {
    val um = glGetUniformLocation (program, "modelviewProjection")
    prval vbox pf_u = pf_u_matrix
    val () = u_matrix := um
  } // end of [where]
  val () = let
      extern castfn __leak1 (x: GLprogram): void
      extern castfn __leak2 (x: GLshader): void
    in
      __leak1 (program); __leak2 (vertShader); __leak2 (fragShader)
    end
in
(*
